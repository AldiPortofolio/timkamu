...
class GetAllTransactionDetailAction extends Controller
{
    /**
    ...
    public function __invoke()
    {
        ...
        if($username) {
            ...
            if($type === 'mlbb' || $type === 'pubgm' || $type === 'freefire' || $type === 'token') {
                ...
                $dataUserTransaction = Transaction::where('desc', 'like', '%'.$idPelanggan.'%')->whereNotIn('item_id', [1, 2, 78])->whereNotIn('payment_type', ['BP', 'COIN'])->paginate(100);
            } else {
                $dataUserTransaction = Transaction::where('phone', $idPelanggan)->whereNotIn('item_id', [2, 141])->whereNotIn('payment_type', ['BP', 'COIN'])->where('status', '<>', 'UNPAID')->paginate(100);
            }
        }
