...
{{--					</div>--}}
                    <div id="data-rules">
                        <div class="outright-category" style="display: none">
                            <div class="upper">
                                <div class="title">
                                    Pemenang M2WC MLBB
                                </div>
...
<div class="init-odds d-flex" style="display: none !important;">
    ...
    <div class="odds-item-action unlocked justify-content-center cursor-ptr scale-onclick bg040 odds-item-row d-flex align-items-center bd50">
        <img src="{{ asset('icons/arrow-right-white.svg') }}" class="icon locked-odds">
    </div>
</div>
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-mid font-size-16 grey-10">
					...
					<p>Waktu pengumuman pemenang <span class="white-10">{{$event->name}}</span> adalah <span class="white-10">{{\Carbon\Carbon::parse($event->end_date)->format('d M Y')}}</span>.</p>
        ...
        function fetchData() {
            $.ajax({
                ...
                success: function(result) {
                    ...
                    // dataRules.find('div.outright-category').not(':first').remove();
                    let betCategories = dataRules.find('div.outright-category:first');
                    let initOdds = $('.init-odds');
                    for(let i = 0; i < result.event_bingo.length; i++){
                        let checkEl = $('#bet_categories_'+result.event_bingo[i].id).length;
                        if(checkEl > 0){
                            let betCategori = $('#bet_categories_'+result.event_bingo[i].id);
                            betCategori.find('.title').text(result.event_bingo[i].name)
                            betCategori.find('.meta').text(result.event_bingo[i].display);
                            betCategori.find('span.default').text(result.event_bingo[i].rules.length + " prediksi tersedia");
                            let betOdds = betCategori.find('.bets');
                            let rules = result.event_bingo[i].rules;
                            for (let j = 0; j < rules.length; j++){
                                let checkOddsEl = $('#odds-rules-'+rules[j].id).length;
                                if(checkOddsEl > 0){
                                    let oddItemAction = $('#odds-rules-'+rules[j].id).find('.odds-item-action');
                                    oddItemAction.attr('data-bonus', rules[j].value_detail.bonus);
                                    oddItemAction.attr('data-type', 'bonus');
                                    oddItemAction.attr('data-id', rules[j].id);
                                    if(rules[j].value_detail.bonus_locked === "1"){
                                        oddItemAction.addClass('locked');
                                        oddItemAction.removeClass('unlocked');
                                        oddItemAction.find('img').attr('src', '{{ asset('icons/lock-white.svg') }}');
                                    }else if( rules[j].value_detail.bonus_locked === "0"){
                                        oddItemAction.removeClass('locked');
                                        oddItemAction.addClass('unlocked');
                                        oddItemAction.find('img').attr('src', '{{ asset('icons/arrow-right-white.svg') }}');
                                    }
                                    continue;
                                }
                                let cloneOdds = initOdds.clone().removeClass('init-odds').addClass('odds-item').attr('id', 'odds-rules-'+rules[j].id).show();
                                cloneOdds.find('.odds-item-name > span').text(rules[j].value_detail.name)
                                cloneOdds.find('.odds-item-bonus > span').text(rules[j].value_detail.bonus + '%');
                                let oddItemAction = cloneOdds.find('.odds-item-action');
                                oddItemAction.attr('data-type', 'bonus');
                                oddItemAction.attr('data-id', rules[j].id);
                                oddItemAction.attr('data-bonus', rules[j].value_detail.bonus);
                                if(rules[j].value_detail.bonus_locked === "1"){
                                ...
                                betOdds.append(cloneOdds);
                            }
                            continue;
                        }
                        let cloneBetCategories = betCategories.clone().attr('id', 'bet_categories_'+result.event_bingo[i].id).show();
                        cloneBetCategories.find('.title').text(result.event_bingo[i].name)
                        cloneBetCategories.find('.meta').text(result.event_bingo[i].display)
                        cloneBetCategories.find('span.default').text(result.event_bingo[i].rules.length + " prediksi tersedia");
                        let betOdds = cloneBetCategories.find('.bets');
                        let rules = result.event_bingo[i].rules;
                        for(let j = 0; j < rules.length; j++){
                            let cloneOdds = initOdds.clone().removeClass('init-odds').addClass('odds-item').attr('id', 'odds-rules-'+rules[j].id).show();
                            cloneOdds.find('.odds-item-name > span').text(rules[j].value_detail.name)
                        ...
                        dataRules.append(cloneBetCategories);
                    }
...
<script>
	generalCountdown("{{ \Carbon\Carbon::now()->format('M d, Y H:i:s') }}", "{{ \Carbon\Carbon::parse($event->end_date)->format('M d, Y H:i:s') }}");
