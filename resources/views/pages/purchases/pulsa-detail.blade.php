...
@php
	$paket = '';
	if(app('request')->input('type') === 'telkomsel') {
		$paket = 'Telkomsel';
	} else if(app('request')->input('type') === 'xl') {
		$paket = 'XL';
	} else if(app('request')->input('type') === 'tri') {
		$paket = 'Tri';
	} else if(app('request')->input('type') === 'google-play') {
		$paket = 'Google Play Voucher';
	}
@endphp
@section('page_title', "Top up - ".$paket)
@section('body_class', 'mana-ui isi-pulsa-'.app('request')->input('type'))
...
<section id="page-content">
		...
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-md-8 col-lg-6">
					<h1 class="rajdhani-bold">@if(app('request')->input('type') === 'google-play'){{ $paket }}@else Paket {{ $paket }} @endif</h1>
				</div>
			</div> 
		</div>
	</div>
	...
	<div id="pulsa-panel" class="mb-20">
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-md-8 col-lg-6">
					<h4 class="font-size-18 mb-20">2. Pilih paket</h4>

					<div class="swiper-container">
						<div class="swiper-wrapper">
                            @if(app('request')->input('type') === 'telkomsel')
							<div class="swiper-slide swiper-panel-item active" data-target="panel1">
								<i data-feather="box" class="icon"></i><a href="#" class="active">Pulsa TSEL</a>
							</div>
							<div class="swiper-slide swiper-panel-item" data-target="panel2">
								<i data-feather="box" class="icon"></i><a href="#">TSEL Flash Kuota</a>
                            </div>
                            @elseif(app('request')->input('type') === 'xl')
                            <div class="swiper-slide swiper-panel-item active" data-target="panel1">
								<i data-feather="box" class="icon"></i><a href="#" class="active">XL Xtra Combo + YouTube</a>
							</div>
							@elseif(app('request')->input('type') === 'tri')
                            <div class="swiper-slide swiper-panel-item active" data-target="panel1">
								<i data-feather="box" class="icon"></i><a href="#" class="active">Tri Kuota</a>
							</div>
							@elseif(app('request')->input('type') === 'google-play')
                            <div class="swiper-slide swiper-panel-item active" data-target="panel1">
								<i data-feather="box" class="icon"></i><a href="#" class="active">Google Play</a>
							</div>
                            @endif
						</div>
					</div>
	...
	<div id="shop-item-shelf" class="mb-50">
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-md-8 col-lg-6">
					@if(app('request')->input('type') === 'telkomsel')
					<div class="shop-panel active panel1">
						<div class="shelf-container d-flex flex-wrap align-items-center">
							@foreach ($items as $item)
                                @php
                                    if(strpos($item['name'], 'Hari')) {
                                        continue;
                                    }
                                @endphp
								<div class="shelf-item shelf-full nominal-choice-btn d-flex scale-onclick @if($item['active'] === '0') unavailable @endif" data-name="{{ $item['name'] }}" data-id_rupiah="{{ $item['id_rupiah'] }}" data-id_lp="{{ $item['id_lp'] }}" data-rupiah="{{ $item['rupiah'] }}" data-lp="{{ $item['lp'] }}">
									<div class="shelf-item-inner d-flex bg020 bd20 justify-content-center">
										<span class="o5 mr-1">Pulsa</span>&nbsp;&nbsp;<span class="money money-14 money-inline right">{{ $item['name'] }}</span>
									</div>
								</div>
							@endforeach
						</div>
					...
					<div class="shop-panel panel2">
						<div class="shelf-container d-flex flex-wrap align-items-center">
							@foreach ($items as $item)
                                @php
                                    if(strpos($item['name'], 'IDR ') === 0) {
                                        continue;
                                    }
                                    $explode = explode('Hari', $item['name']);
                                    $days = str_replace(' ', ' Hari', $explode[0]);
                                    $product = str_replace(' ', '', $explode[1]);
                                @endphp
								<div class="shelf-item shelf-full nominal-choice-btn d-flex scale-onclick @if($item['active'] === '0') unavailable @endif" data-name="{{ $item['name'] }}" data-id_rupiah="{{ $item['id_rupiah'] }}" data-id_lp="{{ $item['id_lp'] }}" data-rupiah="{{ $item['rupiah'] }}" data-lp="{{ $item['lp'] }}">
									<div class="shelf-item-inner d-flex bg020 bd20 justify-content-center">
										<span class="money money-14 money-inline right mr-1">{{ $product }}</span>&nbsp;&nbsp;<span class="o5 font-size-14">{{ $days }}</span>
									</div>
								</div>
							@endforeach
						</div>
                    ...
                    <div class="shop-panel active panel1">
						<div class="shelf-container d-flex flex-wrap align-items-center">
							@foreach ($items as $item)
								<div class="shelf-item shelf-half nominal-choice-btn d-flex scale-onclick @if($item['active'] === '0') unavailable @endif" data-name="{{ $item['name'] }}" data-id_rupiah="{{ $item['id_rupiah'] }}" data-id_lp="{{ $item['id_lp'] }}" data-rupiah="{{ $item['rupiah'] }}" data-lp="{{ $item['lp'] }}">
									<div class="shelf-item-inner d-flex bg020 bd20 justify-content-center">
										<span class="money money-14 money-inline right">{{ $item['name'] }}</span>
									</div>
								</div>
							@endforeach
						</div>
                    ...
                    <div class="shop-panel active panel1">
						<div class="shelf-container d-flex flex-wrap align-items-center">
							@foreach ($items as $item)
                                ...
                                @endphp
								<div class="shelf-item shelf-100 shelf-full nominal-choice-btn d-flex scale-onclick @if($item['active'] === '0') unavailable @endif" data-name="{{ $item['name'] }}" data-id_rupiah="{{ $item['id_rupiah'] }}" data-id_lp="{{ $item['id_lp'] }}" data-rupiah="{{ $item['rupiah'] }}" data-lp="{{ $item['lp'] }}">
									<div class="shelf-item-inner d-flex bg020 bd20 justify-content-center">
										<span class="poetsenone-reg mr-1">{{ $explode[0] }}</span><span class="o5"> + {{ $explode[1] }}</span>
									</div>
								</div>
							@endforeach
						</div>
			...
			<div class="row justify-content-center">
				<div class="col-md-8 col-lg-6">
					...
					<div class="form-group mana-control mb-50">
						...
						<a href="{{ url('purchase/detail?name=pulsa') }}" class="mana-btn-54 mana-btn-opaque">Pilih provider lain</a>
					</div>
			...
			<div class="row justify-content-center">
				<div class="col-md-8 col-lg-6">
					...
					<div class="bd20 bg020 grey-10 d-flex align-items-center justify-content-center mb-20 p5030 font-size-14">
						<p class="mb-0">Pastikan kamu memiliki <span class="money money-inline money-14 white-10 right"><span>LP</span><img src="{{ asset('img/currencies/lp.svg') }}"></span> atau saldo yang cukup sebelum melakukan Top Up Pulsa. Silahkan <a href="/mana/recharge-lp">recharge LP</a> untuk mengisi ulang saldo <span class="money money-inline money-14 white-10 right"><span>LP</span><img src="{{ asset('img/currencies/lp.svg') }}"></span> kamu.</p>
					</div>
		...
		$('body').on('click', '.buy-diamond-btn', function(e) {
            ...
            setTimeout(function(){
				if(paymentType == 'lp') {
					...
					var urlString = ("{{ url('purchase?name=pulsa&id=') }}" + parseInt(idLP))
					urlString = (urlString+"&phone="+phone).replace(/&amp;/g, '&');
					location.href = urlString;
				} else {
					if(paymentType == 'bypass') {
                        var urlString = ("{{ url('purchase?name=pulsa-bypass&id=') }}" + parseInt(idRupiah))
						urlString = (urlString+"&phone="+phone).replace(/&amp;/g, '&');
                        location.href = urlString;
					} else {        
