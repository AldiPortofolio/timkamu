                    ...
                    <div class="right">
						<div class="curr-stat">
							<div class="inside">
								...
								<p>{{ number_format(Auth::user()->total_lp, 0, '.', ',') }} <img src="{{ asset('img/currencies/lp.svg') }}" class="curr-icon"></p>
							</div>
						</div>
						<div class="curr-stat">
							<div class="inside">
								<h5>Total Gain <br>(past month)</h5>
								<p>+{{ number_format($credit_transactions, 0, '.', ',') }} <img src="{{ asset('img/currencies/lp.svg') }}" class="curr-icon"></p>
							</div>
						</div>
						<div class="curr-stat">
							<div class="inside">
								<h5>Total Spending <br>(past month)</h5>
								<p>-{{ number_format($debit_transactions, 0, '.', ',') }} <img src="{{ asset('img/currencies/lp.svg') }}" class="curr-icon"></p>
							</div>
						</div>
						<div class="curr-recharge">
							<a href="{{ url('purchase/detail?name=lp') }}" class="lp">
								Recharge
							</a>
...
<style type="text/css">
	.mobile--profile-lp .area-info--box .curr-icon {
		margin-right: 8px;
	}
	.mobile--profile-lp .curr-recharge a {
		line-height: 60px;
	}
...
<div class="mobile--profile-lp">
    
    @include('includes.profile-mobile-head')

    <div class="mobile--profile--area-selector">
        <i data-feather="more-vertical" class="icon"></i>Transaksi LP
    </div>
    
    <div class="area-info">
        <div class="area-info--box">
            <div class="info-header">Current balance</div>
            <span><img src="{{ asset('img/currencies/lp.svg') }}" class="curr-icon">{{ number_format(Auth::user()->total_lp, 0, '.', ',') }}</span>
        </div>
        <div class="area-info--box">
            <div class="info-header">Total gain (past month)</div>
            <span><img src="{{ asset('img/currencies/lp.svg') }}" class="curr-icon">+{{ number_format($credit_transactions, 0, '.', ',') }}</span>
        </div>
        <div class="area-info--box">
            <div class="info-header">Total spending (past month)</div>
            <span><img src="{{ asset('img/currencies/lp.svg') }}" class="curr-icon">-{{ number_format($debit_transactions, 0, '.', ',') }}</span>
        </div>
        <div class="curr-recharge">
        	<a href="{{ url('purchase/detail?name=lp') }}" class="lp">
        		Recharge
            ...
            <div class="modal-body posrel">
                <div class="message">
					...
					<div class="chc">
                        Nominal convert:
                        <br>
                        <input type="number" id="nominal-convert" > BP
                    </div>
                </div>
            </div>
            <div class="nwcls">
                <a href="#" data-dismiss="modal">Batal</a>
                <a href="#" data-dismiss="modal" class="ctagp" id="btn-confirm-convert">Confirm</a>
            </div>
        </div>
    </div>
</div>
